...
_rather than running one of the CPU-bound processes in-between I/O issues, this results in first process issuing an I/O, and transfering control to the second (via scheduler of course), which then transfers it to the third, which, in turn, transfers it to the fourth, only after which the first process gains control again. this leads to the latter three processes already being terminated when two more I/O are issued by the first process. the more effective utilization of the system resources would have used 21 ticks instead of 31 that this one results in._

*7* Now run the same processes, but with `-I IO_RUN_IMMEDIATE` set, which immediately runs the process that issued the I/O. How does this behavior differ? Why might running a process that just completed an I/O again be a good idea?

_by running the script with the flag above, the more effective (one described in the solution to previous problem) resource utilization is achieved. transfering control back to the process that issued the I/O after it is completed, prevents CPU from idling when waiting for I/O after all of the other process have already terminated (while waiting for each of the I/O, some CPU-bound processes run their instructions)._

*8* Now run with the some randomly generated processess: `-s 1 -l 3:50,3:50` or `-s 2 -l 3:50,3:50` or `-s 3 -l 3:50,3:50`. See if you can predict how the trace will turn out. What happens when you use the flag `-I IO_RUN_IMMEDIATE` vs. `-I IO_RUN_LATER`? What happens when you use `-S SWITCH_ON_IO` vs. `-S SWITCH_ON_END`?

_flag _ `-S SWITCH_ON_END` _proved to be counterproductive. this is due to CPU idling for the entirety of I/O wait time of an I/O-issuing process. when no process issues an I/O this flag has no effect. using `-s SWITCH_ON_IO` appears to be more sensible. as for _`IO_RUN_IMMEDIATE` _vs._ `IO_RUN_LATER`_, similar results in terms of execution ticks can be achieved when CPU-bound vs. I/O-bound time is roughly the same. however, in special cases, like the one described in the previous problem,_ `IO_RUN_IMMEDIATE` _utilizes CPU more effectively._
